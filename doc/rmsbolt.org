...
Each source buffer has its own RMSbolt output and compilation buffers.  Multiple
source files can be compiled at the same time; recompiling one source does not
replace or cancel another source's output.
* Configuring
